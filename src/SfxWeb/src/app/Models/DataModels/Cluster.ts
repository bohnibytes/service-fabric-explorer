...
export class ClusterHealth extends HealthBase<IRawClusterHealth> {
    ...
    public getHealthStateCount(entityKind: HealthStatisticsEntityKind): IRawHealthStateCount {
        if (this.raw && this.raw.HealthStatistics) {
            const entityHealthCount = this.raw.HealthStatistics.HealthStateCountList.find(item => item.EntityKind === HealthStatisticsEntityKind[entityKind]);
...
export class ClusterManifest extends DataModelBase<IRawClusterManifest> {
    ...
    private _isNetworkInventoryManagerEnabled = false;
    private _isBackUpRestoreEnabled = false;
